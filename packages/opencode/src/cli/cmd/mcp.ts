    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in opencode-remote.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .opencode-remote/ subdirectory too)
  const candidates = [path.join(baseDir, "opencode-remote.json"), path.join(baseDir, "opencode-remote.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".opencode-remote", "opencode-remote.json"),
      path.join(baseDir, ".opencode-remote", "opencode-remote.jsonc"),
    )
  }
  ...
  // Default to opencode-remote.json if none exist
  return candidates[0]
}
